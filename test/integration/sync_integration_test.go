...
func TestSyncHealthProvidesMinimalLivenessIntegration(t *testing.T) {
...
func TestAuthHealthDoesNotExposeConfigurationFlagsIntegration(t *testing.T) {
	server, _, _, _ := newSyncIntegrationServer(t, true)

	request, err := http.NewRequest(http.MethodGet, server.URL+"/api/v1/auth/health", nil)
	if err != nil {
		t.Fatalf("new auth health request: %v", err)
	}

	response, err := http.DefaultClient.Do(request)
	if err != nil {
		t.Fatalf("send auth health request: %v", err)
	}
	defer response.Body.Close()

	if response.StatusCode != http.StatusOK {
		t.Fatalf("expected auth health status 200, got %d", response.StatusCode)
	}
	...
	if _, exists := payload["auth_enabled"]; exists {
		t.Fatalf("expected auth_enabled to be absent")
	}
	if _, exists := payload["google_oauth_configured"]; exists {
		t.Fatalf("expected google_oauth_configured to be absent")
	}
	if _, exists := payload["jwt_signing_key_configured"]; exists {
		t.Fatalf("expected jwt_signing_key_configured to be absent")
	}
}
...
func TestSyncObservabilityMetricsIntegration(t *testing.T) {
	...
	metricsBeforeReplay := getSyncMetrics(t, server.URL, token)
	if metricsBeforeReplay.ReplayQueueDepth < 1 {
		t.Fatalf("expected replay_queue_depth >= 1, got %d", metricsBeforeReplay.ReplayQueueDepth)
	}
