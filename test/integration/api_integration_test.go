...
func TestAPIUpdateResourceCategoryIntegration(t *testing.T) {
	router := newIntegrationRouter(t)

	firstCategoryID := createCategory(t, router, `{"name":"Planning","description":"Initial"}`)
	secondCategoryID := createCategory(t, router, `{"name":"Research","description":"Updated"}`)
	resourceID := createResource(t, router, `{"url":"https://example.com/plan","title":"Plan","category_id":"`+firstCategoryID+`"}`)

	updateResponse := doRequest(t, router, http.MethodPatch, "/api/v1/resources/"+resourceID+"/category", `{"category_id":"`+secondCategoryID+`"}`)
	...
	decodeJSON(t, getResource.Body.Bytes(), &payload)
	if payload.Data.CategoryID != secondCategoryID {
		t.Fatalf("expected updated category id %q, got %q", secondCategoryID, payload.Data.CategoryID)
	}
	if payload.Data.CategoryName != "Research" {
		t.Fatalf("expected updated category name Research, got %q", payload.Data.CategoryName)
	}
}

func TestAPITodoUpdateRejectsInvalidDueAtIntegration(t *testing.T) {
	router := newIntegrationRouter(t)
	...
	updateTodo := doRequest(t, router, http.MethodPut, "/api/v1/todos/"+todoID, `{"title":"Updated todo","due_at":"not-a-time"}`)
	assertStatus(t, updateTodo, http.StatusBadRequest)
	assertErrorEnvelope(t, updateTodo.Body.Bytes(), "validation_error")
}

func TestAPIReminderUpdateRejectsInvalidRemindAtIntegration(t *testing.T) {
	router := newIntegrationRouter(t)

	categoryID := createCategory(t, router, `{"name":"Planning","description":"Planning reminders"}`)
	resourceID := createResource(t, router, `{"url":"https://example.com/reminder","title":"Reminder Doc","category_id":"`+categoryID+`"}`)
	reminderID := createReminderWithID(t, router, `{"title":"Initial reminder","message":"follow up","remind_at":"2026-04-22T10:00:00Z","resource_id":"`+resourceID+`"}`)

	updateReminder := doRequest(t, router, http.MethodPut, "/api/v1/reminders/"+reminderID, `{"title":"Updated reminder","remind_at":"bad-time"}`)
	assertStatus(t, updateReminder, http.StatusBadRequest)
	assertErrorEnvelope(t, updateReminder.Body.Bytes(), "validation_error")
}

func TestAPIChatRejectsEmptyMessageIntegration(t *testing.T) {
	router := newIntegrationRouter(t)

	response := doRequest(t, router, http.MethodPost, "/api/v1/chat/commands", `{"message":"   "}`)
	assertStatus(t, response, http.StatusBadRequest)
	assertErrorEnvelope(t, response.Body.Bytes(), "validation_error")

	var payload map[string]string
	decodeJSON(t, response.Body.Bytes(), &payload)
	if payload["error"] != "message is required" {
		t.Fatalf("expected error message to be 'message is required', got %q", payload["error"])
	}
}

func TestAPIChatUnsupportedCommandReturnsHelpIntegration(t *testing.T) {
	router := newIntegrationRouter(t)

	response := doRequest(t, router, http.MethodPost, "/api/v1/chat/commands", `{"message":"bananas on mars"}`)
	...
	decodeJSON(t, response.Body.Bytes(), &payload)
	if payload.Data.Action != "help" {
		t.Fatalf("expected action help, got %q", payload.Data.Action)
	}
	if !strings.Contains(payload.Data.Message, "Unsupported command") {
		t.Fatalf("expected unsupported command message, got %q", payload.Data.Message)
	}
}
