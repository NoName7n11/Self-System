...
// ClassifySkim classifies a resource using the skim-tier inputs (URL + title).
// It tries the primary provider first, then any others, then the fallback.
...
// ClassifyResource classifies a resource using full extracted content
// (title + description + body text). Use this from the deep processing tier
// where the extracted content from Change 6 is available. It shares the same
// provider fan-out as ClassifySkim — the richer signal comes from a fuller
// ClassificationInput.Summary.
func (m *Manager) ClassifyResource(ctx context.Context, input ClassificationInput) (ClassificationOutput, error) {
	return m.classify(ctx, input)
}

// classify runs the provider fan-out: primary → others → fallback. The name of
// the provider that produced the result is stamped into output.Provider.
func (m *Manager) classify(ctx context.Context, input ClassificationInput) (ClassificationOutput, error) {
