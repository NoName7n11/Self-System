...
type CategorySuggestion struct {
	...
	Reason      string
	AutoCreated bool
	// Source is "ai" when a real AI provider produced the classification,
	// or "heuristic" when keyword scoring / URL derivation was used.
	Source string
}

// Classification source constants.
const (
	ClassificationSourceAI        = "ai"
	ClassificationSourceHeuristic = "heuristic"
	ClassificationSourceUser      = "user"
)
		...
		if classifyErr == nil {
			...
			if resolveErr == nil {
				return CategorySuggestion{
					...
					Source:      classificationSourceForProvider(output.Provider),
				}, nil
			}
		}
	}
...
// classificationSourceForProvider maps a provider name to a classification
// source: the heuristic provider maps to "heuristic", any real AI provider to "ai".
func classificationSourceForProvider(provider string) string {
	if strings.EqualFold(strings.TrimSpace(provider), "heuristic") {
		return ClassificationSourceHeuristic
	}
	return ClassificationSourceAI
}
