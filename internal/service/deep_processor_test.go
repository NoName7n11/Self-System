...
func TestDeepProcessorDedupesPendingEnqueue(t *testing.T) {
	resourceSvc, categoryRepo, categorySvc, manager, cleanup := newDeepProcessorTestFixture(t)
	defer cleanup()

	created, err := resourceSvc.Create(context.Background(), CreateResourceInput{
		URL:          "https://example.com/deep-dedupe",
		Title:        "Deep Dedupe",
	...
	if err := processor.Enqueue(DeepTask{ResourceID: created.ID}); err != nil {
		t.Fatalf("enqueue duplicate deep task: %v", err)
	}
	...
	if metrics.DedupedTotal < 1 {
		t.Fatalf("expected deduped total >= 1, got %d", metrics.DedupedTotal)
	}
...
func TestDeepProcessorSkipsRecentReprocess(t *testing.T) {
	resourceSvc, categoryRepo, categorySvc, manager, cleanup := newDeepProcessorTestFixture(t)
	defer cleanup()

	created, err := resourceSvc.Create(context.Background(), CreateResourceInput{
		URL:          "https://example.com/deep-skip",
		Title:        "Deep Skip",
		Summary:      "Initial summary",
		CategoryName: "Research",
	})
	...
	processor := NewDeepProcessor(resourceSvc, categoryRepo, categorySvc, manager, DeepProcessingSettings{
		...
		MaxTasksPerMinute:           120,
		MaxTokensPerDay:             10000,
		MinReprocessIntervalSeconds: 3600,
		ComplexityThreshold:         5,
		LowCostEstimatedTokens:      50,
		HighCostEstimatedTokens:     200,
	})
	...
	waitForDeepCondition(t, 3*time.Second, func() bool {
		metrics := processor.Metrics()
		return metrics.ProcessedTotal >= 1 && metrics.QueueDepth == 0
	})

	if err := processor.Enqueue(DeepTask{ResourceID: created.ID}); err != nil {
		t.Fatalf("enqueue deep task for skip: %v", err)
	}

	waitForDeepCondition(t, 3*time.Second, func() bool {
		metrics := processor.Metrics()
		return metrics.SkippedTotal >= 1
	})
}

func TestDeepProcessorRetainsRecentErrors(t *testing.T) {
	processor := &DeepProcessor{}

	for i := 0; i < deepRecentErrorsCap+5; i++ {
		processor.setLastError(strings.Repeat("e", 1) + "-" + time.Now().Format("150405.000000000") + "-" + strings.Repeat("x", i%3))
	}
	processor.setLastError("final-error")
	processor.setLastError("   ") // empty/whitespace must be ignored

	metrics := processor.Metrics()
	if len(metrics.RecentErrors) != deepRecentErrorsCap {
		t.Fatalf("expected %d retained errors, got %d", deepRecentErrorsCap, len(metrics.RecentErrors))
	}
	if metrics.LastError != "final-error" {
		t.Fatalf("expected LastError to equal most recent non-empty entry, got %q", metrics.LastError)
	}
	if metrics.RecentErrors[len(metrics.RecentErrors)-1] != "final-error" {
		t.Fatalf("expected ring tail to be most recent error, got %q", metrics.RecentErrors[len(metrics.RecentErrors)-1])
	}
}
