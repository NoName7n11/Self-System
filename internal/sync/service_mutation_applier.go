...
func (a *ServiceMutationApplier) applyResourceCreate(ctx context.Context, mutation ReplayMutation) error {
	...
	// Preserve the originating device's entity ID so the resource has the same
	// primary key across all synced devices. Generating a new UUID here would
	// silently break cross-device identity (Finding 9).
