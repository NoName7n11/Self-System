...
func (s *MemoryReplayStore) QueueSnapshot(_ context.Context) (ReplayQueueSnapshot, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return ReplayQueueSnapshot{PendingCount: len(s.pending), OldestEnqueuedAt: oldest}, nil
}
