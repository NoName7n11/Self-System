// DEPRECATED (WS4): sync_offline_queue and sync_replay_store tables are
// superseded by the events table as the canonical source of mutations.
// When events_resource_enabled=true, offline mutations flow through
// ResourceService which appends events; the outbox worker delivers them.
// These tables and this file will be removed after all domains are
// event-sourced (WS5 milestone) and one full release cycle has passed.
package sync
...
func (s *SQLiteReplayStore) QueueSnapshot(ctx context.Context) (ReplayQueueSnapshot, error) {
	var (
		pending int
	...
	if err := row.Scan(&pending, &oldest); err != nil {
		return ReplayQueueSnapshot{}, fmt.Errorf("queue snapshot: %w", err)
	}

	snapshot := ReplayQueueSnapshot{PendingCount: pending}
	if oldest.Valid && strings.TrimSpace(oldest.String) != "" {
		parsed, err := time.Parse(time.RFC3339Nano, oldest.String)
		if err != nil {
			return ReplayQueueSnapshot{}, fmt.Errorf("parse queue oldest enqueued_at: %w", err)
		}
		snapshot.OldestEnqueuedAt = parsed.UTC()
	}
