...
func (r *ResourceRepository) GetByID(ctx context.Context, id string) (*domain.Resource, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT r.id, r.url, r.host, r.title, r.summary, r.category_id, c.name, r.user_override, r.extracted_data, r.created_at, r.updated_at
		FROM resources r
...
func (r *ResourceRepository) Create(ctx context.Context, resource *domain.Resource) error {
	...
	extractedJSON, err := pgMarshalExtractedData(resource.ExtractedData)
	if err != nil {
	...
	`, resource.ID, strings.TrimSpace(resource.URL), strings.TrimSpace(resource.Host),
		...
		strings.TrimSpace(resource.CategoryID), resource.UserOverride,
		extractedJSON, resource.CreatedAt, resource.UpdatedAt)
	if err != nil {
...
func (r *ResourceRepository) Update(ctx context.Context, resource *domain.Resource) error {
	...
	_, err = r.db.ExecContext(ctx, `
		UPDATE resources
		SET url = $1, host = $2, title = $3, summary = $4, category_id = $5, user_override = $6, extracted_data = $7, updated_at = $8
		WHERE id = $9
	`, strings.TrimSpace(resource.URL), strings.TrimSpace(resource.Host),
		strings.TrimSpace(resource.Title), strings.TrimSpace(resource.Summary),
		strings.TrimSpace(resource.CategoryID), resource.UserOverride,
		extractedJSON, resource.UpdatedAt, strings.TrimSpace(resource.ID))
	if err != nil {
...
func (r *ResourceRepository) UpdateExtractedData(ctx context.Context, resourceID string, data domain.ResourceExtractedData) error {
	extractedJSON, err := pgMarshalExtractedData(data)
	if err != nil {
		return fmt.Errorf("marshal extracted_data: %w", err)
	}

	_, err = r.db.ExecContext(ctx, `
		UPDATE resources SET extracted_data = $1, updated_at = $2 WHERE id = $3
	`, extractedJSON, time.Now().UTC(), strings.TrimSpace(resourceID))
	if err != nil {
...
// ── Lifecycle methods (Change 8) — postgres stubs ──────────────────────────
// Full implementations land when the Postgres migration for archive columns ships.
...
func scanResource(row interface{ Scan(dest ...any) error }) (*domain.Resource, error) {
	...
	if extractedJSON != "" && extractedJSON != "{}" {
		_ = json.Unmarshal([]byte(extractedJSON), &item.ExtractedData)
	}
	return &item, nil
}

func pgMarshalExtractedData(data domain.ResourceExtractedData) (string, error) {
