...
- [x] Run `wails generate module` to produce TypeScript bindings in `frontend/src/bindings/`.
- [x] Update Zustand stores to use IPC bindings instead of `fetch()` when running inside Wails (detect via `window.__WAILS__` flag).
- [x] Keep REST client as fallback for browser/dev mode.
- [x] Test all CRUD operations via IPC round-trip.

Deliverables:
- [x] Updated `internal/desktop/app.go` with all IPC method signatures.
- [x] Generated `frontend/src/bindings/` (auto-generated by Wails).
- [x] Updated Zustand stores with IPC/REST toggle.
...
- [x] Frontend Vitest tests: IPC mock → verify stores use IPC path when `window.__WAILS__` is set.
...
- [x] Updated `frontend/src/stores/*.test.ts` — IPC/REST toggle coverage.
- [x] CI smoke gate in workflow.

Done criteria:
- [x] All IPC unit tests pass.
- [x] Frontend store tests cover both IPC and REST transport paths.
...
- [x] Milestone 9D: Windows + Linux CI build pipeline live (WS4 complete).
