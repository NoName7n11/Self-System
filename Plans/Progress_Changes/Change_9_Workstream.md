...
Key tasks:
- [x] System tray: app minimizes to tray instead of closing; tray icon shows sync status. (Verified 2026-06-11: minimize-to-tray + tray icon + right-click Show/Quit menu + left-click restore all work via `HideWindowOnClose` + energye/systray, with all setup/registration done synchronously on a single OS-thread-locked goroutine before entering the message pump — see `internal/desktop/tray.go`. **"tray icon shows sync status" not implemented** — tray has no dynamic icon/state.)
...
Deliverables:
- [x] System tray wiring in `cmd/desktop/main.go` (`HideWindowOnClose: true`) and `internal/desktop/tray.go` (energye/systray `RunWithExternalLoop` with `runtime.LockOSThread()` + synchronous icon/menu setup on one goroutine, `SetOnRClick`/`SetOnClick`, Show/Quit menu, embedded icon).
...
- [x] App minimizes to system tray and can be restored from tray icon. (Re-verified 2026-06-11 after the tray threading fix: close → tray icon remains; right-click → Show/Quit menu appears; left-click and Show both restore the window.)
