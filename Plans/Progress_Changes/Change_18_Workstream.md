...
### Chat fit + task color/order + MAP root + empty-chat prune + RECENTS dock + interactive notifs (Session 71) ✅
`frontend/src/components/chat/ChatDock.tsx`, `frontend/src/components/layout/Sidebar.tsx`, `frontend/src/components/graph/GraphCanvas.tsx`, `frontend/src/components/layout/Topbar.tsx`, `frontend/src/components/resource/ResourceForm.tsx`, `frontend/src/stores/useLayoutStore.ts`, `frontend/src/stores/useChatStore.ts`, `frontend/src/lib/demoData.ts`, `frontend/src/types.ts`, `frontend/src/styles.css`.

- [x] **Chat attachment chips fit the message cell** — `MsgAtts` measures the bubble inner width (`useLayoutEffect`) and shows 1 or 2 chips (capped 2, reserving the `+N more` pill) instead of a fixed `slice(0,2)`; moved `<MsgAtts>` inside `.chat-bubble` in both dock and rail (was a sibling → outside the cell); `.msg-att-row` max-width 74%→100%. Verified: narrow "anime" cell → 1 chip + "+5 more", wide cell → 2 chips + "+4 more".
- [x] **Task status color + order swap** — TO DO now orange `#F0703C`, IN PROGRESS blue `#5B9CF6` (done green), across `STATUS_COLOR`, both `cols` arrays (dock + rail), and MAP `mapStatusColor`; columns reorder to TO DO → IN PROGRESS → DONE. Verified in dock TasksTab (orange TO DO box, blue IN PROGRESS box, correct order).
- [x] **MAP root node expand/contract** — removed the `kind==="root" ? true` force-expand in `buildMapLayout`; `mapExpanded` seeded `{root:true}` and `toggleMapAll` preserves it. Verified: clicking the ROOT TASKS node collapses/expands the tree (chevron ▾/▸).
- [x] **Empty-chat auto-delete** — `pruneEmptyConversations(keepId?)` in the chat store; rail NEW CHAT prunes strays first, thread back button (`leaveConv`) deletes the conversation if it has zero messages. Verified: new chat + leave-without-typing → pruned, only the 3 seeded chats remain.
- [x] **Interactive notifications (generic)** — `NotifAction {label,kind,resourceId?}` on `DemoNotif`; central `runNotifAction` dispatcher in Topbar (one `case` per `NotifActionKind`); `preview_resource` selects the resource, opens the Inspector, and fires `previewSignal` → Inspector enters PREVIEW (via new `firePreview()` + `useEffect`). Action button renders only on an expanded notif carrying an action; demo `n1` → `r3` (PDF). Verified: PREVIEW → Inspector shows Advanced RAG Paper inline preview.
- [x] **RECENTS in the Bottom_Dock** — new `recents` DockTab + `RecentsTab` mirroring the Left_Rail RECENT list (inspect / per-row remove / CLEAR), `.dock-recents*` CSS. Verified: tab lists 5 recents with type swatches; clicking a row opens the Inspector.
- [x] **Reference file untouched** — an accidental edit to `Redesign/Self Systems.dc.html` (design reference only) was reverted; all changes are in `frontend/src/`.
