...
**Eval correction (Session 57, 2026-06-13):** WS3's evaluation was reworked because it could not actually measure the "≥5% lift over weighted scoring" promotion criterion — the old `evaluateAccuracy` was circular (learned weights checked against the signals they were trained on) and compared against the previous model artifact rather than the weighted-scoring scheme. `scripts/gbus_train/main.go` now reads raw `gbus_signal` events and runs a prequential (expanding-window) top-1/top-3 ranking evaluation: each labeled instance (`manual_classification`/`category_correction`) is ranked against rankers trained only on strictly-earlier events, comparing `baselineWeights` (Outline plain weighted-sum) vs `gbusWeights` (log-dampened, delete-penalised, confidence-discounted). `-promote` gates on `gbusTop1 >= baselineTop1 + 0.05` with a minimum test count; covered by new `scripts/gbus_train/main_test.go`. The metric is now trustworthy but still un-run (no signal data: `gbus.enabled: false`). This realises the "GBUS as category-affinity prior" framing — see `Change_10_Workstream.md` WS3 methodology note.
...
# Change 16: GBUS Schema Alignment — User-Scoped Signals & Confidence Tracking
Date: 2026-06-13

## What to do
Close the implementation gap between the Phase 3 GBUS spec and `internal/gbus`: stamp `user_id`/`session_id` on every signal and feature row (the spec required these from the start, but the scaffold implementation omitted them, leaving features aggregated per-category/resource with no user scoping), and add `evidence_count`/`confidence` to category features so inference/training can discount low-evidence categories.

## What we did
Added `UserID`/`SessionID` to `GBUSSignalPayload` (`internal/gbus/signals.go`), with `SignalEmitter.Emit()` defaulting `user_id` to `DefaultUserID = "local"` and `session_id` to a per-process UUID generated in `NewSignalEmitter`. Added `ExplicitIntentWeightThreshold = 0.5` and `ConfidenceEvidenceThreshold = 10` constants to `internal/domain/entities.go` (aliased from `internal/gbus` for callers there) — kept in `domain` rather than `gbus` to avoid an import cycle (`sqlite repo -> gbus -> eventstore -> sqlite repo`). Extended `domain.GBUSCategoryFeature`/`GBUSResourceFeature` with `UserID`, and `GBUSCategoryFeature` with `EvidenceCount`/`Confidence`. Updated `domain.GBUSFeatureStore` interface and `internal/repository/sqlite/gbus_repository.go` so `UpsertCategoryFeature`/`GetCategoryFeatures`/`UpsertResourceFeature`/`GetResourceFeatures` all take a leading `userID` argument; `UpsertCategoryFeature` now also increments `evidence_count` (when `weight >= ExplicitIntentWeightThreshold`) and recomputes `confidence = MIN(1.0, evidence_count / ConfidenceEvidenceThreshold)` in the SQL upsert. Added migration v4 (`gbus_user_scoped_features`) adding `user_id` (default `'local'`), `evidence_count`, `confidence` columns plus indexes — no primary-key rebuild needed since `(category_id/resource_id, signal_type)` remains a valid PK for the single-user case. Updated `internal/gbus/aggregator.go` to default `userID` from `payload.UserID` and pass it through to both upsert calls. Fixed the `inMemoryFeatureStore` test mock in `internal/gbus/aggregator_test.go` to match the new interface signatures. Following a review that flagged `confidence`/`evidence_count` as captured-but-unused, wired them into the training pipeline (WS6): `scripts/gbus_train/main.go`'s `listAll` now selects the columns and `computeCategoryWeights` discounts each category's score by `MIN(1, aggregate_evidence / ConfidenceEvidenceThreshold)` before normalization — applied at category level since passive signal types structurally carry per-row confidence 0. Runtime inference intentionally still reads only the trained `CategoryWeights` (confidence's effect is baked in at training time, not applied per-request). The training-time effect is latent until the first real run (Change 10 WS3, still pending).

## Why this approach
The Phase 3 spec (`Plans/Progress/Phase_3_GBUS_Signals_Feature_Store.md` section 3) already specified `user_id`/`session_id` as required/optional event fields — this was an implementation gap, not a spec gap, so no spec rewrite was needed beyond a gap note and a new decision entry (section 8/9). Kept the existing `(category_id|resource_id, signal_type)` primary keys rather than rebuilding tables around a `(user_id, ...)` composite key — `user_id` is constant `'local'` for every row in the current single-user app, so a plain column + index is sufficient and a real composite-key migration is deferred until multi-user/sync actually lands. Deliberately did NOT implement multi-axis signal decomposition, hourly aggregation cadence, session-pattern modeling, or embeddings — confirmed as premature for a single-user, low-volume system; `confidence`/`evidence_count` give inference/training a cheap way to discount sparse categories without that complexity.

# Change 18: Instrument UI Full Rebuild (Knowledge Graph + Left_Rail)
Date: 2026-06-29

## What to do
Re-baseline the frontend UI on `Redesign from scratch_7/8` (the authoritative design) and bring each component to fidelity, section by section. This Change is tracked in detail in `Change_18_Workstream.md`; this record summarizes the graph + Left_Rail fidelity passes.

## What we did
Knowledge Graph (`frontend/src/components/graph/GraphCanvas.tsx`): brought the custom canvas-2D force sim to design parity — sparse 7-edge CATLINKS + resource↔resource `con` links (organic, non-symmetric layout), counter-based node radii, screen-space projection + DPR for crisp rendering. Fixed two stability/feel regressions: (1) a phantom 12s "refresh/flicker" caused by the no-backend offline poll rebuilding a fresh `demoResourcesAsItems()` array each cycle — fixed at source in `useResourceStore.loadResources` with a `resourcesSignature()` short-circuit that keeps the same array reference when silent-reload data is unchanged; (2) "rough/irregular" drag — reverted an alpha-decay experiment back to the design's full-force-every-frame `step()` (equilibrium + 0.9 damping keeps it static at rest yet fluid on drag) and switched the event model to the design's (window-level `mousemove`/`mouseup` so drags track off-canvas, `getBoundingClientRect` coords, circular hit test). Verified via Playwright: idle pixel-stable, drag follows cursor with neighbour reaction then re-converges to rest.

Left_Rail (`frontend/src/components/layout/Sidebar.tsx`, `frontend/src/styles.css`, `useResourceStore.ts`): (1) Library section "open in dock" now targets the `library` dock tab (was `categories`); (2) RECENT now supports tap-to-collapse / hold-1.5s-to-clear with an animated progress fill, backed by a new `clearRecents()` store action (mirrors design `startHoldRecent`); also dropped the `deriveRecents` fallback that masked the clear; (3) CATEGORY NODES rows compacted — removed the per-row colored type badge (title only) for long node lists; (4) the home nav "open in dock" affordances (CHAT/TASKS/LIBRARY) restyled to a 24×24 bordered box matching `.rail-icon-btn`, keeping hover-reveal, for uniform iconography. All verified live via Playwright; `npm run build` clean.

Main_Content (`GraphCanvas.tsx`, `Topbar.tsx`, `icons.tsx`, `useLayoutStore.ts`, `styles.css`): (1) fixed the GRAPH going blank after a MAP roundtrip — the canvas is now permanently mounted (hidden under overlays) with a `viewRef`-gated rAF loop, so its DPR backing store keeps its size (the conditional unmount had left it 300×150); (2) reworked MAP from a static CSS-scaled list into an interactive mind-map matching the design §11 — `buildMapLayout()` (ROOT→category→task tree, Bézier edges) rendered in a pannable/zoomable world with drag-pan, wheel-zoom, expand toggles, EXPAND/COLLAPSE ALL, zoom pill, and FIT-recenter; (3) added a new PROGRESS "RECENTLY COMPLETED" feature — a persisted multi-select column-customize menu (DATE·TIME·TYPE·CATEGORY) plus LIBRARY-style per-row removal; (4) fixed the notifications panel — store-backed `notifList`, working `CLEAR` (empties + keeps panel open) and `MUTE`/`MUTED` toggle, per-row removal, and a three-state bell (plain/alert/muted via new `IcoBellAlert`/`IcoBellOff`). Two follow-up polish passes: the notification rows no longer resize on hover (the `×` is absolute, overlaying the timestamp), and MAP is now both sharp at rest and trail-free while panning — solved by permanently compositing the unscaled `.map-viewport` (`translateZ(0)` + `contain:paint`) while leaving the scaled world un-promoted so it re-rasterizes crisply.

Bottom_Dock (`ChatDock.tsx`, `useTaskStore.ts`, `useLayoutStore.ts`, `styles.css`): (1) CHAT — wired the composer `+` to a real file picker (staged removable chips) and removed the redundant YOU/ASSISTANT labels; (2) TASKS — uniform-height cards (flexible for long titles) and a create-form CATEGORY picker that leads with AUTO, shows 3 inline categories, and a scrollable MORE dropdown for the rest; (3) LIBRARY — a persisted multi-select column menu (TYPE·CATEGORY·DATE) like MAP/PROGRESS; (4) CATEGORIES — the design's split-ADD caret → add-options menu, NEW CATEGORY row (creates a live category card), and a working ingest file picker. All verified live via Playwright; `npm run build` clean.

Dock/Rail polish (`ChatDock.tsx`, `styles.css`): five follow-up fixes against the design and reference shots. (1) CATEGORIES ingest is now horizontally centered (new `.ingest-inner` 560px wrapper) and the add-options dropdown anchors right-aligned 4px below the ADD/caret instead of opening far away (img_6 not img_7) — it was anchored to a full-width wrapper before. (2) Dock CHAT bubbles use the design's 12px row gap and 74%-capped width (were 6px/80% — cramped and over-stretched). (3) Left_Rail LIBRARY/RECENT rows keep their fixed design height and scroll when the window shrinks (added `flex:none` so they stop flex-shrinking inside the flex-column list; img_7). (4) All scrollbars are now uniform 8px with the design thumb colors via a single global `::-webkit-scrollbar` rule — six per-section 3px overrides and the `scrollbar-width:thin` line (which overrode webkit in WebView2) were removed (§1.7, img_8). (5) The residual MAP pan-trail from the root TASKS node (img_9) is fixed by promoting each `.map-node` to its own compositor layer (`will-change:transform`) so moving nodes never leave residue in the viewport tiles; Chrome rasters promoted layers at screen scale so the earlier blur does not return (verified sharp at 160% and 45%). All verified live via Playwright; `npm run build` clean.

Notifications + chat attachments + Right_Rail (`Topbar.tsx`, `ChatDock.tsx`, `Sidebar.tsx`, `ResourceForm.tsx`, `useChatStore.ts`, `useResourceStore.ts`, `useFilteredResources.ts`, `types.ts`, `styles.css`): six fixes against design §7/§8.2/§8.3/§9.3. (1) Notifications expand in place on click (2-line clamp → full, long bodies scroll within the panel) so large/actionable notifs are readable. (2) Staged chat files now ride the sent message: `ChatMessage.atts` + a shared `MsgAtts` component renders the first 2 as chips and a `+N more` chip with a hover tooltip listing all — wired into both the dock and Left_Rail chat. (3) Right_Rail OPEN→PREVIEW: links keep OPEN, every other type shows a PREVIEW toggle (accent when active) that swaps AI SUMMARY for an inline type-aware preview mock (PDF/DOC page, NOTE card, IMAGE frame). (4) The bottom "ASK ABOUT THIS RESOURCE…" composer was removed. (5) ARCHIVE and DELETE are functional — a new `archivedLib` set drives archive/restore; archiving removes the node from library + graph, opens the ARCHIVE tab, and is restorable (EDIT left as a no-op). (6) The Inspector meta badge now highlights the resource's category node (name + category color) instead of the file type (type already shows in the preview well), with the duplicate category tag chip dropped. All verified live via Playwright; `npm run build` clean.

PREVIEW sticky fix (`ResourceForm.tsx`, `demoData.ts`): corrected the PREVIEW reset logic — the previous `useEffect` cleared `previewActive` on every node selection change; the fix narrows it to only clear when the newly selected resource is a link (links show OPEN, never PREVIEW), so PREVIEW persists across all previewable types and only auto-ends on a link click. Also added a long confirmation notification (`n0` — "UI changes deployed") to `DEMO_NOTIFS` as both a visible confirmation artifact and a functional test for the Session 67 expand/scroll behavior.

## Why this approach
Fixing the graph refresh at the data source (`loadResources` signature guard) rather than inside the canvas component stops the churn for every memo/effect keyed on `resources`, not just the graph. Matching the design's physics and event model exactly (rather than a d3-style alpha sim) is the lazy, correct path — the design already encodes a stable-at-rest/fluid-on-drag system, so divergence only reintroduced the problems. Left_Rail changes are presentation/interaction only; backend, IPC, and store shapes are unchanged.
