...
WS4 delivered: `build-desktop` job in `.github/workflows/release.yml` building Windows and Linux desktop binaries via `wails build` on release tags; `wails-dev`/`wails-build-windows`/`wails-build-linux`/`gbus-train` Makefile targets updated to `cd cmd/desktop` first. CI build never run green — `wails build` not yet proven locally either.

WS5 delivered: `internal/desktop/app_test.go` with unit tests for the IPC methods using in-memory service stubs (build tag removed). Full `go test ./...` passes with zero regressions. Frontend Vitest IPC-mock coverage for task/reminder stores not yet added.
