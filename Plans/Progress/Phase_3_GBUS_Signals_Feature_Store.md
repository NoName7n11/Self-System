...
- Confidence/evidence tracking (Change 16): `gbus_category_features` rows now carry `evidence_count` (count of signals with weight >= `ExplicitIntentWeightThreshold` = 0.5, i.e. "explicit intent" signals) and `confidence` (`MIN(1.0, evidence_count / ConfidenceEvidenceThreshold)`, threshold = 10). The training pipeline (`scripts/gbus_train`) consumes this: `computeCategoryWeights` discounts each category's score by its aggregate evidence-based confidence before normalization, so categories backed only by passive signals are damped relative to ones with explicit manual classifications/corrections. Runtime inference (`internal/gbus/inference.go`) applies the resulting pre-trained `CategoryWeights` and does not read confidence directly — by design, since confidence's effect is baked into training, not applied per-request. This avoids needing the full multi-axis signal decomposition or hourly aggregation cadence from sections 2.6/5 — both remain deferred as premature for a single-user, low-volume system.
...
- `gbus_category_features` and `gbus_resource_features` gained a `user_id` column (default `'local'`), added via migration v4 rather than a primary-key rebuild — acceptable for the current single-user app; a real multi-user migration is a known follow-up if/when Phase 2+ multi-user/sync lands.
  - **Not multi-user isolation-safe yet:** primary keys stay `(category_id, signal_type)` / `(resource_id, signal_type)` and upserts `ON CONFLICT` on those keys, then overwrite `user_id`. Two users writing the same `category+signal` would merge into one row (last-writer-wins on `user_id`), not isolate. Safe only while `user_id` is constant `'local'`; isolation requires the deferred composite-key rebuild.
  - **`confidence` is a training-time input, not a runtime inference input:** `scripts/gbus_train` bakes the confidence discount into the trained `CategoryWeights`; `internal/gbus/inference.go` reads those weights and never reads `confidence` per-request.
  - **Training is still global (cross-user):** `scripts/gbus_train` aggregates by `category_id` only (no `user_id` in `listAll` or `computeCategoryWeights`), so a multi-user rollout would train a shared profile even after storage isolation is fixed — conflicting with this spec's user-category affinity target. Keying training and `GBUSModel.CategoryWeights` by `(user_id, category_id)` is deferred alongside the composite-key storage rebuild.
