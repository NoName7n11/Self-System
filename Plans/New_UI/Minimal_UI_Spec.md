...
- **Primary_Nav** (`663:13`): 3 items — **Chat**, **Tasks**, **Library**. (See §4 for why "Search" and "Knowledge Graph" were removed.) Each nav row now carries a hover-revealed **"open-in-split" affordance** (Material Symbol `bottom_panel_open` — panel frame with an up-arrow inside, prepended at the row's extreme left, rendered at ~0.45 opacity as a hover hint) — clicking it opens that section as a tab in Main_Content's Split_Section (§3). Applies to Chat/Tasks/Library only, **not** Search. *(Session 60)*
