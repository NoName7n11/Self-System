...
- **Primary_Nav** (`663:13`): 3 items — **Chat**, **Tasks**, **Library**. (See §4 for why "Search" and "Knowledge Graph" were removed.) Each nav row now carries an **"open-in-split" affordance** (Material Symbol `open_in_new_down`, pinned at the **right end** of the row via a flex spacer) — clicking it opens that section as a tab in Main_Content's Split_Section (§3). Present on all three rows (`Nav_Chat`/`Nav_Tasks`/`Nav_Library`), shown in the static mock; at runtime it is **hover-gated** (visible only when the row is hovered — opacity 0→1). Applies to Chat/Tasks/Library only, **not** Search. *(Session 60)*
