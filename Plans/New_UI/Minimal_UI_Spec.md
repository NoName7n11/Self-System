...
- **Primary_Nav** (`663:13`): 3 items — **Chat**, **Tasks**, **Library**. (See §4 for why "Search" and "Knowledge Graph" were removed.) Each nav row now carries a hover-revealed **"open-in-split" affordance** (Material Symbol `open_in_new_down`, pinned at the **right end** of the row via a flex spacer) — clicking it opens that section as a tab in Main_Content's Split_Section (§3). It is **visible only on hover**: in the static mock it's shown on `Nav_Chat` (opacity 1, hovered demo) and hidden on `Nav_Tasks`/`Nav_Library` (opacity 0); structure is identical across rows so a Hover variant / prototype can toggle opacity 0→1. Applies to Chat/Tasks/Library only, **not** Search. *(Session 60)*
