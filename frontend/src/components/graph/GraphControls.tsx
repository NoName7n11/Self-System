...
export function deriveCategoryOptions(resources: ResourceItem[]) {
  return [...new Set(resources.map((item) => item.categoryName.trim()).filter((item) => item !== ""))].sort(
...
export default function GraphControls({ resources }: GraphControlsProps) {
  ...
  const categories = useMemo(() => deriveCategoryOptions(resources), [resources]);
