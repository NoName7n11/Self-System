...
export function buildGraph(resources: ResourceItem[]): GraphData {
...
export function toFinite(value: number | undefined): number {
...
export function getNodeColor(node: GraphNode, selectedResourceId: string | null): string {
...
export default function GraphCanvas({ resources }: GraphCanvasProps) {
  ...
  useEffect(() => {
    ...
    const api = {
      selectResource: (resourceId: string) => {
        const match = graphData.nodes.find((node) => node.resourceId === resourceId);
        if (match) {
      ...
      selectCategory: (categoryName: string) => {
        ...
        const match = graphData.nodes.find(
          (node) => node.kind === "category" && node.categoryName?.toLowerCase() === normalized,
        );
        if (match?.categoryName) {
          setCategoryFilter(match.categoryName.toLowerCase());
        }
      },
    };

    const target = window as unknown as { __graphTest?: typeof api };
    target.__graphTest = api;

    return () => {
      if (target.__graphTest === api) {
        delete target.__graphTest;
      }
    };
  }, [graphData, selectResource, setCategoryFilter]);
  ...
  const nodeLabel = (nodeObject: unknown) => getNodeLabel(nodeObject as GraphNode);
